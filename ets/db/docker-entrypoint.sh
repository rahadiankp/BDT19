...
if [ "$1" = 'mysqld' -a -z "$wantHelp" ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		echo "Configuring for Group Replication"
		"${mysql[@]}" <<-EOSQL
			SET @@SESSION.SQL_LOG_BIN=0;
		...
		echo "Installing GROUP REPLICATION plugin"
		"${mysql[@]}" <<-EOSQL
			SET @@SESSION.SQL_LOG_BIN=0;
			INSTALL PLUGIN group_replication SONAME 'group_replication.so';
		EOSQL

		#echo "Configuring for ProxySQL Monitoring"
		# "${mysql[@]}" < addition_to_sys.sql

		echo "Creating monitor:monitor user for ProxySQL"
		"${mysql[@]}" <<-EOSQL
			SET @@SESSION.SQL_LOG_BIN=0;
		...
		if [ "$MYSQL_USER" -a "$MYSQL_PASSWORD" ]; then
			...
			echo 'SET @@SESSION.SQL_LOG_BIN=0; FLUSH PRIVILEGES ;' | "${mysql[@]}"
		fi
